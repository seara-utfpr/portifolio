...
function getMimeType($image_src)
{
	...
	return $mime_type;	
		...
		if (isset($_FILES['bpm_file']['tmp_name']))
		{			
        ...
        $sql = "SELECT * FROM imagem_processo WHERE id='$id_imagem'";
        ...
        if (mysqli_num_rows($result))
		{
			...
			if($nome_arquivo!='')
			{
				$sql = "UPDATE imagem_processo SET nome='$nome_arquivo' WHERE id=$id_imagem";
				mysqli_query($conn, $sql);
			}
			if($bpm !='')
			{
				$bpm = base64_encode(file_get_contents(addslashes($bpm)));
				$sql = "UPDATE imagem_processo SET img='$bpm' WHERE id=$id_imagem";
				mysqli_query($conn, $sql);
			}
        ...
        else
		{
                ...
                echo "alert('Error Impossível achar Imagem para atualização')";
        }
        mysqli_close($conn);
?>