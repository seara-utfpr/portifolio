...
/* fim validacao de sessao */
 ...
 function gera_menu($cod_ul_pai,$id_do_pai){
  include 'dbh.php';
  //gera um menu ul com submenus
   //exemplo: gera_menu("<ul>","    ",0)
  $query  ="SELECT id_processo,nome_processo FROM processo WHERE parent_id ='".$id_do_pai."' and situacao_id= 2 order by nome_processo asc ";
  ...
  if($result){
      while($linha = mysqli_fetch_assoc($result)){
          ...
          //testando se tem filhos
              $query = "SELECT id_processo,nome_processo FROM processo WHERE parent_id ='".$linha['id_processo']."' and situacao_id=2 order by nome_processo asc";
              $recfilho = mysqli_query($conn,$query);
              if(mysqli_num_rows($recfilho)>0)
              {
                  $ret.= '<span class="arrow"></span>';
                  $ret.= '</a></li>'; 
                  $ret .= gera_menu('<ul class="sub-menu collapse" id="id'.$linha['id_processo'].'">',$linha['id_processo']);
              }
              $ret.= '</a></li>'; 
          //fim filhos          
      }
...
<!------ Menu ---------->
    ...
    <div class="brand"><center>Processos</center></div>

    <i class="fa fa-bars fa-2x toggle-btn" data-toggle="collapse" data-target="#menu-content"></i>

      <div class="menu-list">
            <ul id="menu-content" class="menu-content collapse out">
			    <? echo $menu; //colocando o menu na posição correta.?>
            </ul>
     </div>
