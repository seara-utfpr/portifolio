...
{ 
	...
	$query  ="SELECT parent_id,nome_processo,id_processo FROM processo WHERE id_processo ='".$id_processo."' and situacao_id =1"; 
		...
		if($macro == 'S')
		{
			...
			$sql_relacionados = "SELECT DISTINCT p.id, p.id_processo, p.nome_processo AS nome_processo, CONCAT(SUBSTRING(p.descricao,1,50),'...') AS descricao, p.versao AS versao ,
							...
							FROM `processo` AS p LEFT JOIN portfolio_de_processos_situacao s ON s.id=p.situacao_id
												 LEFT JOIN processo  mp ON mp.id_processo=p.parent_id WHERE p.parent_id = '".$id_processo."' AND p.situacao_id= 1 ORDER BY p.nome_processo ASC";
